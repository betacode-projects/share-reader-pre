...
function get_file_info(){
    $.ajax({
        ...
        type: 'post',
        data: {'send_secret_token': $.cookie('sender_secret')}
    })
    .done(function(data) {
        console.log(data);
...
$('#move-permission').html('<a href="'+ url +'move-permission.html?secret='+ $.cookie('sender_secret') +'">ダウンロード</a>');
$('#download-qr').qrcode({text: url +'download-link.html?sender='+ $.cookie('sender'), correctLevel : 2});
$('#move-permission').qrcode({text: url +'move-permission.html?secret='+ $.cookie('sender_secret'), correctLevel : 2});
